...
TDD: written before the implementation. Run with:
    python -m pytest tests/scripts/test_fetch_program.py -v
...
def _import_cli():
    """Import fetch_program.main fresh (avoids stale monkeypatches across tests)."""
    import importlib
...
def test_huntr_dispatch_writes_scope_yaml(tmp_programs, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--fixture", str(FX_HUNTR / "repo_acme-org_acme-pkg.html"),
    ])
    rc = fp.main()
    assert rc == 0
...
def test_ghsa_dispatch_writes_scope_yaml(tmp_programs, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--fixture", str(FX_GHSA / "repos_acme-org_acme-repo.json"),
    ])
    rc = fp.main()
    assert rc == 0
...
def test_ibb_dispatch_writes_scope_yaml(tmp_programs, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--venue", "ibb-h1",
        "--identifier", "django",
        "--fixture", str(FX_IBB / "structured_scopes_django.json"),
    ])
    rc = fp.main()
    assert rc == 0
...
def test_draft_result_writes_draft_yaml_not_scope_yaml(tmp_programs, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--venue", "ibb-h1",
        "--identifier", "django",
        "--fixture", str(FX_IBB / "forbidden_403.json"),
    ])
    rc = fp.main()
    assert rc == 1
    # Draft file present
...
def test_schema_invalid_result_writes_draft_not_scope_yaml(tmp_programs, monkeypatch, capsys):
    ...
    # Mutate the dict in-place (not replace it) so argparse choices={sorted(FETCHERS)} stays valid.
    original_huntr = fp.FETCHERS["huntr"]
    fp.FETCHERS["huntr"] = MagicMock(return_value=FetchResult(
    ...
    try:
        ...
        fp.FETCHERS["huntr"] = original_huntr
...
def test_fetch_failure_writes_nothing(tmp_programs, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--venue", "huntr",
        "--identifier", "acme-org/acme-pkg",
        "--fixture", str(FX_HUNTR / "garbage.html"),
    ])
    rc = fp.main()
    assert rc == 1
...
def test_scope_violation_propagates_no_write(tmp_programs, monkeypatch, capsys):
    ...
    original_huntr = fp.FETCHERS["huntr"]
    fp.FETCHERS["huntr"] = MagicMock(side_effect=ScopeViolation(
...
def test_existing_scope_blocks_without_force(tmp_programs, monkeypatch, capsys):
...
def test_force_overwrites_existing_scope(tmp_programs, monkeypatch):
